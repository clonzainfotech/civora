        ...
        <thead>
            ...
            @forelse($appointment as $row)
                <tr>
                    ...
                    <td class="data-font seperator">{{ !empty($row->time) ? \Carbon\Carbon::parse($row->date)->format('d-m-Y') . ' ' .  \Carbon\Carbon::parse($row->time)->format('h:i') : \Carbon\Carbon::parse($row->date)->format('d-m-Y') }}</td>
