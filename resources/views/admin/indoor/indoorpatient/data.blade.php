    ...
    <tbody>
        ...
        @forelse($indoorData as $patients)
            <tr class="indoorpatient" data-id="{{encrypt($patients->id)}}">
                ...
                <td><a href="#" class="sticker-link text-dark" data-toggle="modal" data-target="#sticker-modal" data-date="{{$patients->doa_date}}" data-appointmentid="{{'0'}}">{{(!empty($patients->doa_date)) ? \Carbon\Carbon::parse($patients->doa_date)->format('d-m-Y') : '-' }}</a></td>
