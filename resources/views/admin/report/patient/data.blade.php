<table class="table m-b-0 table-hover font" id="report-table">
    <thead>
        ...
        @if (count($patientReportOpd) > 0)
            <tr>
                <td colspan="6" class="sub-headline">{{ $patientReportOpd[0]->getPatientsDetails['name'] }}</td>
            </tr>
        @endif
    </thead>
    <tbody>
        ...
        @if (count($patientReportOpd) > 0)
            ...
            @endphp
            @forelse($patientReportOpd as $row)
                <tr data-id="{{encrypt($row->id)}}">
                    ...
                    <td>
                        @if(isset($row['amount']))
                            {{ucfirst(!empty($row->charge_type) ? $chargeType[$row->charge_type] : null)}}
                        @else
